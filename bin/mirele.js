...
const $export = {
    ...
    async onMessage({context, dialogId}) {
        ...
        const text = getTextualMessageOrNull(context);

        if (text) {
            if ($use.string.getUrls(text)) {
                const yt = $use.string.getUrls(text).find(i => i.startsWith("https://www.youtube.com/watch?v="))

                if (yt) {
                    replyWithText(dialogId, "i will look now, wait a second")

                    let videoId = $helpers.youtubeParser(yt)

                    replyWithText(dialogId, "ID" + videoId)

                    let outSnippet = $use.essent.snippet('ytcc', {videoId})
            ...
            const prism1 = $use.essent.snippet("prism1",{
                message: text
            })

            const out = $use.essent.task(
              "text-classification",
            ...
            replyWithText(
                dialogId,
                JSON.stringify({prism1, out}, undefined, 2)
            )
        }
    }
};
